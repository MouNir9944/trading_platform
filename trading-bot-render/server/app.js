...
export function createApp({ orderManager, auth } = {}) {
  ...
  api.get("/market/price", handle(500, async (req) => {
    ...
    const mode = binance.getMode();
    const price = await cache.get(`price:${mode}:${symbol}`, 1500, () =>
      binance.getCurrentPrice(binance.getClient(mode), symbol));
    return { symbol, price };
  }));
  ...
  api.get("/market/candles", handle(500, async (req) => {
    ...
    const mode = binance.getMode();
    const candles = await cache.get(`candles:${mode}:${symbol}:${interval}:${limit}`, 2000, () =>
      binance.getRecentCandles(binance.getClient(mode), symbol, interval, limit));
